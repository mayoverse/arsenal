...
To specify a caption unfortunately requires two parameter specifications:
...
For `compare.data.frame` objects, the output function in `write2()` is `summary()`.
For `summary.compare.data.frame` objects, the output function is `print()`.
