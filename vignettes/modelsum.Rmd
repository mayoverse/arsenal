...
variables such as age and sex. Locally, the SAS macros `%modelsum`, `%glmuniv`, and `%logisuni`
...
resource is available at [rmarkdown.rstudio.com](rmarkdown.rstudio.com). 
...
the variable names. The three single quotes are often located above the tab key.
...
It is important to understand how R treats the `digits` argument.  Here are some summaries for
the variable `pi`.  Note that with 4 digits, the number after the decimal point changes after
multiplying pi by 10 or 100.  However, the `nsmall` option specifies the number of values after
the decimal point.  The two can be used together (see the help file for `format` for more details on how that works).
