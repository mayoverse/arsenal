...
head(as.data.frame(noby)) ## or use as.data.frame(noby)
```
...
```{r}
head(as.data.frame(summary(noby)))
```
...
withnames <- freqlist(tab.ex, labelTranslations = c(arm = "Treatment Arm", sex = "Gender", mdquality.s = "LASA QOL"), digits = 0)
summary(withnames)
```
...
withby <- freqlist(tab.ex, strata = c("arm","sex"))
summary(withby)
...
```{r, results = 'asis'}
summary(noby, labelTranslations = labs)
```
...
xftbl <- xtable(as.data.frame(summary(noby)), 
