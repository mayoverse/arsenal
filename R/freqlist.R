...
#' @param na.options a character string indicating how to handling missing values: 'include' (include values with NAs in counts and percentages), 'showexclude' (show NAs but exclude from cumulative counts and all percentages), 'remove' (remove values with NAs); default is 'include'
#' @param digits a single number indicating the number of digits for percentages (passed to \code{\link{round}}; default is 2
#' @param labelTranslations an optional character string of labels to use for variable levels when printing 
...
freqlist <- function(tab, sparse = FALSE, na.options = c('include', 'showexclude', 'remove'), digits = 2, labelTranslations = NULL, groupBy = NULL, ...)
{
  ...
  internalTable <- function(data, na.options = na.options, digits = digits) {
    ...
    if (na.options == 'remove') {
      ...
      cumFreq <- with(data, cumsum(Freq))
      freqPct <- with(data, 100 * Freq / sum(Freq))
