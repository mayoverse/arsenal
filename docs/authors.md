# Authors and Citation

## Authors

- **Ethan Heinzen**. Author, maintainer.

- **Jason Sinnwell**. Author.

- **Elizabeth Atkinson**. Author.

- **Tina Gunderson**. Author.

- **Gregory Dougherty**. Author.

- **Patrick Votruba**. Contributor.

- **Ryan Lennon**. Contributor.

- **Andrew Hanson**. Contributor.

- **Krista Goergen**. Contributor.

- **Emily Lundt**. Contributor.

- **Brendan Broderick**. Contributor.

- **Maddie McCullough**. Artist.

## Citation

Source:
[`DESCRIPTION`](https://github.com/mayoverse/arsenal/blob/HEAD/DESCRIPTION)

Heinzen E, Sinnwell J, Atkinson E, Gunderson T, Dougherty G (2026).
*arsenal: An Arsenal of 'R' Functions for Large-Scale Statistical
Summaries*. R package version 3.7.0,
<https://github.com/mayoverse/arsenal>.

    @Manual{,
      title = {arsenal: An Arsenal of 'R' Functions for Large-Scale Statistical Summaries},
      author = {Ethan Heinzen and Jason Sinnwell and Elizabeth Atkinson and Tina Gunderson and Gregory Dougherty},
      year = {2026},
      note = {R package version 3.7.0},
      url = {https://github.com/mayoverse/arsenal},
    }
